rule ivar_consensus:
    input:
        ref_fa = "results/patched_reference/{sample}/patched_reference.fasta", #reference coming from the output of the patching.smk
        ref_index = "results/index/{sample}_reference.mmi",
    ...
    params:
        ...
        base_qual = config["ivar_params"].get("mpileup_min_base_quality", 20), #-Q
        map_qual = config["ivar_params"].get("mpileup_min_map_quality", 20),#-q
        ivar_qual = config["ivar_params"].get("ivar_min_quality", 20)#-q

    conda:
        "../envs/ivar.yaml"
    shell:
        """
        (samtools mpileup -A -d {params.max_depth} -Q {params.base_qual} -q {params.map_qual} -f {input.ref_fa} {input.sorted_bam} | \
        ivar consensus -p {params.prefix} -q {params.ivar_qual} -t {params.min_freq} -m {params.min_cov} -n N) > {log} 2>&1
        """
...
rule concat_consensus:
    ...
    run:
        ...
        with open(output[0], "w") as out: #renaming the multifasta file headers so that tree has sample names
            for fasta in input:
